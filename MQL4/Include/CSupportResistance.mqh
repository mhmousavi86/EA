...
//|               Copyright 2017, Erwin Beckers Modified by Mhmousavi|
...
private:
   ...
   void DrawLine(int& lineCnt, SRLine* line, int maxTouches, int maxBars, string key, color colorText,color &colors[], bool showAgeLabels, bool showLastTouch)
   {
      ...
      if (showAgeLabels)
      {
         ...
         if (yrs >= 1 )
         {
            ...
            string txt= IntegerToString((int)line.Price);
            if (showLastTouch) txt=txt+ IntegerToString((int)line.Price);
   ...
   //+------------------------------------------------------------------+
   double getPriceLine2(int& lineCnt, SRLine* line, int maxTouches, int maxBars, string key, color colorText,color &colors[], bool showAgeLabels, bool showLastTouch)
   {
      string name = "@"+key+" (" + DoubleToStr(line.Price,5) + ") " + TimeToStr(line.StartDate,TIME_DATE)+" - " + TimeToStr(line.EndDate,TIME_DATE);
...
//+------------------------------------------------------------------+
    ...
    ClearAll(key);
      ...
      if (!showAllSRLines)
      {
         ...
         for (int i=0; i < _maxLine; ++i)
         {
            ...
            {
               if (i-2 >= 0) {
                  priceLine = getPriceLine2(lineCnt, _lines[i-2], maxTouches, maxBars, key, colorText,colors,  showAgeLabels,  showLastTouch);
                  }
               break;
            }
         }
      ...
      double getSecondResistancePrice( string key)
   {
   ...
   //+------------------------------------------------------------------+
   double getFirstResistance( string key, color colorText, color &colors[], bool showAgeLabels, bool showLastTouch, bool showAllSRLines)
   {
    ClearAll(key);
   ...
   double getFirstResistancePrice( string key)
   {
      color        Colors[]     = {Red,Red, Red,Red};
       double priceLine = 0;
      // draw lines
      ...
      {  
         ...
         for (int i=0; i < _maxLine; ++i)
         {
            if ( _lines[i].Price > marketPrice)
            {
                priceLine = getPriceLine2(lineCnt, _lines[i], maxTouches, maxBars, key, White,Colors,  false,  false);
               break;
            }
         }
   ...
   double getSecondSupportPrice( string key)
   {
   ...
   double getSecondSupport( string key, color colorText, color &colors[], bool showAgeLabels, bool showLastTouch, bool showAllSRLines)
   {
    ClearAll(key);
   ...
   double getFirstSupport( string key, color colorText, color &colors[], bool showAgeLabels, bool showLastTouch, bool showAllSRLines)
   {
    ClearAll(key);
      ...
      if (!showAllSRLines)
      {
         ...
         for (int i=0; i < _maxLine; ++i)
         {
            if ( _lines[i].Price > marketPrice)
            {
                priceLine = getPriceLine2(lineCnt, _lines[i], maxTouches, maxBars, key, colorText,colors,  showAgeLabels,  showLastTouch);
               break;
            }
         }
      }
   ...
   double getFirstSupportPrice( string key)
   {
