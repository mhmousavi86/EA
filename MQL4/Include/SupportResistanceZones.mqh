...
extern color color_support_verified = Gold;
...
extern color color_resist_verified  = Gold;
...
double FastDnPts[3000], FastUpPts[3000];
double SlowDnPts[3000], SlowUpPts[3000];
   ...
   if (NewBar() == true)
   {
     ...
     if (zone_show_info == true)
     {
        for (int i=0; i<zone_count; i++)
        {
           ...
           double dif=MathMin(MathAbs(Time[2]-Time[3]),MathAbs(Time[1]-Time[2]));
           datetime dtStart=Time[0]+dif*zone_label_shift+dif*ShiftEndRight;
           
           double vpos = zone_hi[i] - (zone_hi[i] - zone_lo[i]) / 2 ;
             
           string s = "SSSR#"+i+"LBL";
           ObjectCreate(s, OBJ_TEXT, 0, 0, 0);
           ObjectSet(s, OBJPROP_TIME1, dtStart);
           ObjectSet(s, OBJPROP_PRICE1, vpos);
           ObjectSet(s, OBJPROP_ANCHOR, ANCHOR_LEFT);
   ...
   for (int i=0; i<zone_count; i++)
   {
      ...
      else if (zone_type[i] == ZONE_RESIST)
      {
         ...
         lbl = lbl +  "\n" + "Weak 2";
            ...
            lbl = lbl +  "\n" + "ZONE_VERIFIED Verified2" + zone_hi[i];
...
//--- successful execution
 ...
 double getHightZone(string timeframe,
                     int type,
                     int zone)
{
   TimeFrame = timeframe;
   ...
   for (int i=0; i<zone_count; i++)
   {
      ...
      {
         // support zone
         if (type == ZONE_TURNCOAT && zone_strength[i] == ZONE_TURNCOAT) {
            result = zone_hi[i];
           }
         else if (type == ZONE_WEAK && zone_strength[i] == ZONE_WEAK) {
            result = zone_hi[i];
           }
         else if (type == ZONE_VERIFIED && zone_strength[i] == ZONE_VERIFIED) {
            result = zone_hi[i];
           }
         else if (type == ZONE_UNTESTED && zone_strength[i] == ZONE_UNTESTED) {
            result = zone_hi[i];
           }
         else {
            result = zone_hi[i];
 ...
 double getLowZone(string timeframe,
                     int type,
                     int zone)
{
   TimeFrame = timeframe;
